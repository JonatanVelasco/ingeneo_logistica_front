...
export class EntregasComponent implements OnInit {
  ...
  delete(entrega: Entrega): void {
    swal({
      title: 'Está seguro?',
      text: `¿Seguro que desea eliminar al entrega No. ${entrega.id} con número de guia ${entrega.nro_guia} ?`,
      type: 'warning',
      ...
      if (result.value) {
        ...
        this.entregaService.delete(entrega.id).subscribe(
          response => {
            ...
            swal(
              'Entrega Eliminado!',
              `Entrega No. ${entrega.id} con número de guia ${entrega.nro_guia}  eliminada con éxito.`,
              'success'
            )
          }
        )
